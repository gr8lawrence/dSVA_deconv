...
### This file re-perform the analysis on GSE73072 under extant batch effects

# Preprocessing -----------------------------------------------------------

## read the full data
all_data_ls <- readRDS("../dSVA_datasets/GSE73072_processed_batch.rds")
...
## H1N1 data 
h1n1_meta_full <- rbind(all_meta$H1N1_DEE, all_meta$H1N1_DEE3) %>% 
  as_tibble()
h1n1_meta <- h1n1_meta_full %>% 
  ...
  arrange(time_hr)
h1n1_bulk <- cbind(all_bulk_ls$H1N1_DEE, all_bulk_ls$H1N1_DEE3)[, h1n1_meta$geo_accession]
# all.equal(colnames(h1n1_bulk), h1n1_meta$geo_accession)

# table(h1n1_meta_full$ID, h1n1_meta_full$time_hr)

## H3N2 data
h3n2_meta_full <- rbind(all_meta$H3N2_DEE2, all_meta$H3N2_DEE5) %>% 
  as_tibble() 
h3n2_meta <- h3n2_meta_full %>% 
  as_tibble() %>% 
  filter(time_hr == 0 | time_hr == 18 | time_hr == 22 | time_hr == 42 | time_hr == 46 | time_hr == 66 | time_hr == 70 | time_hr == 106 | time_hr == 108) %>%
  filter(infected_ID != "Undecided") %>% # Now we want to clean the samples whose infected_ID is not undecided
  mutate(time_hr_char = as.character(time_hr),
         time_hr_char = fct_reorder(time_hr_char, time_hr)) %>% 
  mutate(time_day = case_when(
    time_hr == 0 ~ "Day 0",
    time_hr == 18 | time_hr == 22 ~ "Day 1",
    time_hr == 42 | time_hr == 46 ~ "Day 2",
    time_hr == 66 | time_hr == 70 ~ "Day 3",
    time_hr == 106 | time_hr == 108 ~ "Day 5"
  )) %>% 
  arrange(time_hr) 
h3n2_bulk <- cbind(all_bulk_ls$H3N2_DEE2, all_bulk_ls$H3N2_DEE5)[, h3n2_meta$geo_accession]

## HRV
hrv_meta <- rbind(all_meta$HRV_DUKE, all_meta$HRV_UVA) %>% 
  as_tibble() %>% 
  ...
  arrange(time_hr) 
hrv_bulk <- cbind(all_bulk_ls$HRV_DUKE, all_bulk_ls$HRV_UVA)[, hrv_meta$geo_accession]

## RSV
rsv_meta <- all_meta$RSV_DEE1 %>% 
  as_tibble() %>% 
  ...
  arrange(time_hr) 
rsv_bulk <- all_bulk_ls$RSV_DEE1[, rsv_meta$geo_accession]
...
for (i in 1:length(timepoints)) {
  ...
  study_plt_ls[[i]] <-  autoplot(h1n1_pca_R_hat, data = Meta_t, colour = "study") +
    labs(subtitle = paste("Timepoint =", timepoint)) +
    theme_base(base_size = 18, base_family = "Times") +
    theme(legend.position = "bottom", 
          axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1),
          plot.background=element_blank()) +
    scale_color_manual(name = "Study", values = c("#ffd380", "#2c4875")) 
    # scale_color_manual(name = "Infection Status", values = c("#ffd380", "#8a508f", "#2c4875")) 
  
  infect_plt_ls[[i]] <-  autoplot(h1n1_pca_R_hat, data = Meta_t, colour = "infected_ID") +
    labs(subtitle = paste("Timepoint =", timepoint)) +
    theme_base(base_size = 18, base_family = "Times") +
    theme(legend.position = "bottom", 
          axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1),
...
ggpubr::ggarrange(plotlist = study_plt_ls, nrow = 3, ncol = 2, common.legend = FALSE)
ggsave("GSE73072_batch_plots/H1N1_Residual_by_Study.pdf", width = 7.5, height = 9.5)

ggpubr::ggarrange(plotlist = infect_plt_ls, nrow = 3, ncol = 2, common.legend = FALSE)
ggsave("GSE73072_batch_plots/H1N1_Residual_by_Infected.pdf", width = 7.5, height = 9.5)
...
line_plt_nnls <- h1n1_P_nnls_long %>% 
  ...
  ggplot(aes(x = time_hr, y = Proportion, col = infected_ID, linetype = study)) +
  geom_point(size = 1) +
  ...
  scale_linetype(name = "Study") +
  guides(color = guide_legend(nrow = 1))
ggsave("GSE73072_batch_plots/H1N1_Props_Line_Plots_NNLS.pdf", line_plt_nnls, width = 9.5, height = 7.5)
ggsave("GSE73072_batch_plots/H1N1_Props_Line_Plots_NNLS.tiff", line_plt_nnls, width = 19, height = 15)
...
ggsave("GSE73072_batch_plots/H1N1_Props_Boxplots_NNLS.pdf", boxplot_nnls, width = 9.5, height = 7.5)
ggsave("GSE73072_batch_plots/H1N1_Props_Boxplots_NNLS.tiff", boxplot_nnls, width = 19, height = 15)
...
line_plt_cutoff <- h1n1_P_cutoff_long %>% 
  ...
  scale_linetype(name = "Study") +
  guides(color = guide_legend(nrow = 1))
ggsave("GSE73072_batch_plots/H1N1_Props_Line_Plots_Cutoff.pdf", line_plt_cutoff, width = 9.5, height = 7.5)
...
boxplot_cutoff <- h1n1_P_cutoff_long %>% 
  ...
  scale_fill_manual(name = "Infected", values = c("#ffd380", "#2c4875")) +
  guides(fill = guide_legend(nrow = 1))
ggsave("GSE73072_batch_plots/H1N1_Props_Boxplots_Cutoff.pdf", boxplot_cutoff, width = 9.5, height = 7.5)
...
for (i in 1:length(timepoints)) {
  ...
  study_plt_ls[[i]] <-  autoplot(H3N2_pca_R_hat, data = Meta_t, colour = "study") +
    labs(subtitle = paste("Timepoint =", timepoint)) +
    theme_base(base_size = 18, base_family = "Times") +
    theme(legend.position = "bottom", 
          axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1),
  ...
  infect_plt_ls[[i]] <-  autoplot(H3N2_pca_R_hat, data = Meta_t, colour = "infected_ID") +
    labs(subtitle = paste("Timepoint =", timepoint)) +
    theme_base(base_size = 18, base_family = "Times") +
    theme(legend.position = "bottom", 
          axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1),
...
ggpubr::ggarrange(plotlist = study_plt_ls, nrow = 3, ncol = 2, common.legend = FALSE)
ggsave("GSE73072_batch_plots/H3N2_Residual_by_Study.pdf", width = 7.5, height = 9.5)

ggpubr::ggarrange(plotlist = infect_plt_ls, nrow = 3, ncol = 2, common.legend = FALSE)
ggsave("GSE73072_batch_plots/H3N2_Residual_by_Infected.pdf", width = 7.5, height = 9.5)

## check correlation
lapply(H3N2_P_ls_cutoff, function(x) cor(x$Gamma_hat, sig_marker))
# lapply(H3N2_P_ls_tw, function(x) cor(x$Gamma_hat, sig_marker))
...
P_cutoff <- Reduce(cbind, lapply(H3N2_P_ls_tw, function(x) x$P_hat)) # change to TW
...
line_plt_nnls <- H3N2_P_nnls_long %>% 
  ...
  scale_linetype(name = "Study") +
  guides(color = guide_legend(nrow = 1))
ggsave("GSE73072_batch_plots/H3N2_Props_Line_Plots_NNLS.pdf", line_plt_nnls, width = 9.5, height = 7.5)
ggsave("GSE73072_batch_plots/H3N2_Props_Line_Plots_NNLS.tiff", line_plt_nnls, width = 19, height = 15)
...
boxplot_nnls <- H3N2_P_nnls_long %>% 
  ...
  # scale_linetype(name = "Study") +
  guides(fill = guide_legend(nrow = 1))
ggsave("GSE73072_batch_plots/H3N2_Props_Boxplots_NNLS.pdf", boxplot_nnls, width = 9.5, height = 7.5)
ggsave("GSE73072_batch_plots/H3N2_Props_Boxplots_NNLS.tiff", boxplot_nnls, width = 19, height = 15)
...
line_plt_cutoff <- H3N2_P_cutoff_long %>% 
  ...
  scale_linetype(name = "Study") +
  guides(color = guide_legend(nrow = 1))
ggsave("GSE73072_batch_plots/H3N2_Props_Line_Plots_TW.pdf", line_plt_cutoff, width = 9.5, height = 7.5)
...
boxplot_cutoff <- H3N2_P_cutoff_long %>% 
  ...
  scale_fill_manual(name = "Infected", values = c("#ffd380", "#2c4875")) +
  guides(fill = guide_legend(nrow = 1))
ggsave("GSE73072_batch_plots/H3N2_Props_Boxplots_TW.pdf", boxplot_cutoff, width = 9.5, height = 7.5)
...
for (i in 1:length(timepoints)) {
  ...
  study_plt_ls[[i]] <-  autoplot(hrv_pca_R_hat, data = Meta_t, colour = "study") +
    labs(subtitle = paste("Timepoint =", timepoint)) +
    theme_base(base_size = 18, base_family = "Times") +
    theme(legend.position = "bottom", 
          axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1),
          plot.background=element_blank()) +
    # scale_color_manual(name = "Infection Status", values = c("#ffd380", "#2c4875")) 
    scale_color_manual(name = "Study", values = c("#ffd380", "#8a508f", "#2c4875"))
  
  infect_plt_ls[[i]] <-  autoplot(hrv_pca_R_hat, data = Meta_t, colour = "infected_ID") +
    labs(subtitle = paste("Timepoint =", timepoint)) +
    theme_base(base_size = 18, base_family = "Times") +
    theme(legend.position = "bottom", 
          axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1),
...
ggpubr::ggarrange(plotlist = study_plt_ls, nrow = 3, ncol = 2, common.legend = FALSE)
ggsave("GSE73072_batch_plots/HRV_Residual_PC1_by_Study.pdf", width = 7.5, height = 9.5)

ggpubr::ggarrange(plotlist = infect_plt_ls, nrow = 3, ncol = 2, common.legend = FALSE)
ggsave("GSE73072_batch_plots/HRV_Residual_PC1_by_Infected.pdf", width = 7.5, height = 9.5)
...
P_cutoff <- Reduce(cbind, lapply(hrv_P_ls_tw, function(x) x$P_hat)) # change to TW
...
line_plt_nnls <- hrv_P_nnls_long %>% 
  ...
  scale_linetype(name = "Study") +
  guides(color = guide_legend(nrow = 1))
ggsave("GSE73072_batch_plots/HRV_Props_Line_Plots_NNLS.pdf", line_plt_nnls, width = 9.5, height = 7.5)
ggsave("GSE73072_batch_plots/HRV_Props_Line_Plots_NNLS.tiff", line_plt_nnls, width = 19, height = 15)
...
boxplot_nnls <- hrv_P_nnls_long %>% 
  ...
  guides(fill = guide_legend(nrow = 1))
ggsave("GSE73072_batch_plots/HRV_Props_Boxplots_NNLS.pdf", boxplot_nnls, width = 9.5, height = 7.5)
ggsave("GSE73072_batch_plots/HRV_Props_Boxplots_NNLS.tiff", boxplot_nnls, width = 19, height = 15)
...
line_plt_cutoff <- hrv_P_cutoff_long %>% 
  ...
  scale_linetype(name = "Study") +
  guides(color = guide_legend(nrow = 1))
ggsave("GSE73072_batch_plots/HRV_Props_Line_Plots_TW.pdf", line_plt_cutoff, width = 9.5, height = 7.5)
...
boxplot_cutoff <- hrv_P_cutoff_long %>% 
  ...
  scale_fill_manual(name = "Infected", values = c("#ffd380", "#8a508f", "#2c4875")) +
  guides(fill = guide_legend(nrow = 1))
ggsave("GSE73072_batch_plots/HRV_Props_Boxplots_TW.pdf", boxplot_cutoff, width = 9.5, height = 7.5)
...
ggsave("GSE73072_batch_plots/RSV_Residual_PC1.pdf", width = 7.5, height = 9.5)
...
line_plt_nnls <- rsv_P_nnls_long %>% 
  ...
  scale_color_manual(name = "Infected", values = c("#ffd380", "#2c4875")) +
  guides(color = guide_legend(nrow = 1))
ggsave("GSE73072_batch_plots/RSV_Props_Line_Plots_NNLS.pdf", line_plt_nnls, width = 9.5, height = 7.5)
ggsave("GSE73072_batch_plots/RSV_Props_Line_Plots_NNLS.tiff", line_plt_nnls, width = 19, height = 15)
...
boxplot_nnls <- rsv_P_nnls_long %>% 
  ...
  scale_fill_manual(name = "Infected", values = c("#ffd380", "#2c4875")) +
  guides(fill = guide_legend(nrow = 1))
ggsave("GSE73072_batch_plots/RSV_Props_Boxplots_NNLS.pdf", boxplot_nnls, width = 9.5, height = 7.5)
ggsave("GSE73072_batch_plots/RSV_Props_Boxplots_NNLS.tiff", boxplot_nnls, width = 19, height = 15)
...
line_plt_cutoff <- rsv_P_cutoff_long %>% 
  ...
  scale_color_manual(name = "Infected", values = c("#ffd380", "#2c4875")) +
  guides(color = guide_legend(nrow = 1))
ggsave("GSE73072_batch_plots/RSV_Props_Line_Plots_Cutoff.pdf", line_plt_cutoff, width = 9.5, height = 7.5)
...
boxplot_cutoff <- rsv_P_cutoff_long %>% 
  ...
  scale_fill_manual(name = "Infected", values = c("#ffd380", "#2c4875")) +
  guides(fill = guide_legend(nrow = 1))
ggsave("GSE73072_batch_plots/RSV_Props_Boxplots_Cutoff.pdf", boxplot_cutoff, width = 9.5, height = 7.5)
