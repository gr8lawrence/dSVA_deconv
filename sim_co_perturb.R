...
## make title
title = "Case-control Structure (Co-Variation)"
...
for (p in p_sig) {
  for (p_c in p_change) {
    for (b in 1:B) {
      ...
      P_hat_ls <- list()
      true_data <- dSVA_model_sim_pert(m, n, K, q, p, lambda, err = err, p_pert = p_c)
      
      ## estimate n_comp once for each setting
      if (b == 1) q_hat_mat[which(p_sig == p), which(p_change == p_c)] <- dSVA_n_comp <- estimate_n_comp(Y = true_data$Y, Theta = true_data$X, method = "be", B = 49, seed = 100)
