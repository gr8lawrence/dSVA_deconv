...
for (gamma in gamma_seq) {
  ...
  boxplot(result_df$cor_nnls, result_df$cor_pnnls, result_df$cor_known, 
          main = paste0("gamma = ", gamma), xlab = "Method", ylab = "Cor",
          col = c("orange", "azure", "lightpink"))
  axis(1, at = c("1", "2", "3"), labels = c("NNLS", "PNNLS", "Known"))
  boxplot(result_df$mse_nnls, result_df$mse_pnnls, result_df$mse_known, 
...
```{r simulation_code_st, echo = FALSE}
B <- 100 # run the simulation B times
par(mfrow = c(1, 2))
result_matrix <- matrix(nrow = B, ncol = 6)
...
While we draw the PCA plot of the effects, being in the nonnegative orthant means that the large variation in the latent variable does directly affect the results. It depends on the direction where the latent variables take effects. See the plot below:
...
```{r sim_param_var, echo = FALSE, eval = FALSE}
## Other sketch area
n <- 20
m <- 1000
K <- 5
...
lambda <- 3
mu <- 5
sigma_seq <- 3 * sqrt(5)^(1:4)
```

```{r simulation_code_var, echo = FALSE, eval=FALSE}
B <- 100
result_matrix <- matrix(nrow = B, ncol = 6)
par(mfrow = c(3, 2)))
for (sigma in sigma_seq) {
  ...
  for (b in 1:B) {
    true_data <- dSVA_model_sim_2(m, n, K, p, p_sig, lambda, mu, sigma)
  ...
  result_df <- as_tibble(cbind(1:B, result_matrix))
  colnames(result_df) <- c("b", "cor_nnls", "cor_pnnls", "cor_known", "mse_nnls", "mse_pnnls", "mse_known")
  boxplot(result_df$cor_nnls, result_df$cor_pnnls, result_df$cor_known, xlab = "Method", ylab = "Cor", main = paste("Mu =", mu, "Sigma =", round(sigma, 3) ))
  axis(1, at = c("1", "2", "3"), labels = c("NNLS", "PNNLS", "Known"))
  boxplot(result_df$mse_nnls, result_df$mse_pnnls, result_df$mse_known, xlab = "Method", ylab = "MSE", main = paste("Mu =", mu, "Sigma =", round(sigma, 3) ))
  axis(1, at = c("1", "2", "3"), labels = c("NNLS", "PNNLS", "Known"))
}
}  
