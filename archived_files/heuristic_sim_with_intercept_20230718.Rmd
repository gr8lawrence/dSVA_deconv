---
title: "Simulation to Find Latent Variable Effects with an Intercept"
author: "Tianyi Liu"
date: "2023-07-18"
...
$$
\mathbf{Y}_{m \times n} = \mathbf{A}_{m \times n} + \mathbf{\Theta}_{m \times K} \mathbf{P}_{K \times n} + \mathbf{Z}_{m \times p}\mathbf{D}_{p \times n} + \mathbf{E}_{m \times n}
$$
where $\mathbf{A}_{m \times n}$ is an intercept term $\mathbf{D}_{p \times n} $ and $\mathbf{Z}_{m \times p}$ are latent factors of an , $E\left[ \mathbf{Z}_{m \times p}\mathbf{D}_{p \times n} + \mathbf{E}_{m \times n} \right] = 0$. 

If we write $\mathbf{X} = \left[ \mathbf{I}_{m \times m} \ \mathbf{\Theta}_{m \times K} \right]$ and $\mathbf{B} = \left[ \mathbf{A}^T \ \mathbf{P}^T \right]^T$, this model can be re-written as 
$$
\mathbf{Y}_{m \times n} = \mathbf{X}_{m \times (m + K)} \mathbf{B}_{(m + K) \times n} + \mathbf{Z}_{m \times p}\mathbf{D}_{p \times n} + \mathbf{E}_{m \times n}
$$
...
q_hats_int <- vector(mode = "double", length = B)
q_hats_null <- vector(mode = "double", length = B)
...
for (gamma in gamma_seq) {
  for (b in 1:B) {
    ...
    ## dSVA
    P_dsva <- dsva_ext2_know_q(Y = true_data$Y, X = true_data$X)$P_hat
...
for (gamma in gamma_seq) {
  for (b in 1:B) {
    ...
    ## dSVA
    dsva_ls <- dsva_ext2_know_q(Y = true_data$Y, 
                         X = true_data$X)
