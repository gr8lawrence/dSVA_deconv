...
If the $q$ is unknown, we need to estimate the value of $q$, which could be accomplished by using Horn's parallel analysis (PA).

```{r simulation_code_case_1_q_unknown, echo = FALSE, message = TRUE, warning = FALSE}
...
for (gamma in gamma_seq) {
  for (b in 1:B) {
    ...
    ## dSVA
    dsva_ls <- dsva_ext_know_q(Y = true_data$Y, 
                         X = true_data$X)
    P_dsva <- dsva_ls$P_hat
    # q_hats[b] <- dsva_ls$q_hat
